...
void PublishTankState(byte id)
{
	...
	setHexInt16(buffer, getPressureSensorState(id), 0);
	setHexInt16(buffer, getWaterTankPercent(id), 4);
	buffer[8] = float_switch_states[id] ? '1' : '0';
	setHexInt16(buffer, getBallValveState(id), 9);
...
void PublishSettings()
{
	...
	{
		idx = setHexInt16(buffer, tankVoltageSettings[i].empty, idx);
		idx = setHexInt16(buffer, tankVoltageSettings[i].full, idx);
	}
...
void callback(char* topic, byte* payload, unsigned int len) {
	...
	if (strncmp(topic, "chac/wl/settings2/", 18) == 0)
	{
		...
		tankVoltageSettings[id].empty = readHexInt16(p);
		p += 4;
		tankVoltageSettings[id].full = readHexInt16(p);
		//p += 4;
