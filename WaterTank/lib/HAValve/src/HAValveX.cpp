...
HAValveX::HAValveX(const char* uniqueId, const Features features) :
...
bool HAValveX::setState(const ValveState state, const bool force)
{
...
bool HAValveX::setPosition(const int16_t position, const bool force)
{
...
bool HAValveX::publishState(ValveState state)
{
...
bool HAValveX::publishPosition(int16_t position)
{
...
void HAValveX::handleCommand(const uint8_t* cmd, const uint16_t length)
{
    if (!_commandCallback) {
        return;
    }
